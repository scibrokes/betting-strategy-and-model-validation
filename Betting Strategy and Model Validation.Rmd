...
## Refer to named chunk `read-data-summary-table` for further details.
...
## Annual summry graph.
m <- ddply(mbase$datasets, .(Sess), summarise, Stakes=sum(Stakes)/10000, Return=sum(Return)/10000, PL=sum(PL)/10000) %>% tbl_df
...
dt1 <- dat %>% group_by(Sess, Month) %>% summarise(S.total=sum(Stakes/10000), S.median=median(Stakes/10000), S.mean=round(mean(Stakes/10000),4), S.sd=round(sd(Stakes/10000),4), Count=length(PL/10000), minHKPrcB=min(HKPrice/10000), maxHKPrcB=max(HKPrice/10000), minProbB=min(netProbB), maxProbB=max(netProbB), Return=sum(PL/10000,Stakes/10000), PL=sum(PL/10000), PL.percent=sprintf(sum(PL/10000)/sum(Stakes/10000),fmt="%1.4f%%"), Samples=paste(Stakes/10000, collapse = ",")) %>% mutate(Series = Samples)
names(dt1)[1] <- 'Sess'
ddim <- dim(dt1)
...
m <- ddply(data.frame(Month=as.yearmon(mbase$datasets$Date,'%b'),mbase$datasets), .(Month), summarise, Stakes=sum(Stakes)/10000, Return=sum(Return)/10000, PL=sum(PL)/10000) %>% tbl_df %>% mutate(Month=as.character(Month))
...
dt2 <- dat %>% group_by(Sess, Month, Day) %>% summarise(S.total=sum(Stakes/10000), S.median=median(Stakes/10000), S.mean=round(mean(Stakes/10000),4), S.sd=round(sd(Stakes/10000),4), Count=length(PL/10000), minHKPrcB=min(HKPrice/10000), maxHKPrcB=max(HKPrice/10000), minProbB=min(netProbB), maxProbB=max(netProbB), Return=sum(PL/10000,Stakes/10000), PL=sum(PL/10000), PL.percent=sprintf(sum(PL/10000)/sum(Stakes/10000),fmt="%1.4f%%"), Samples=paste(Stakes/10000, collapse = ",")) %>% mutate(Series = Samples)
names(dt2)[1] <- 'Sess'
ddim <- dim(dt2)
...
#'@ ahBets <- ddply(mbase2, .(HCap, AHOU, Picked), summarise, Stakes=sum(Stakes/10000), S.median=round(median(Stakes/10000),4), S.mean=round(mean(Stakes/10000),4), S.sd=round(sd(Stakes/10000),4), Count=length(PL), Return=sum(PL,Stakes/10000), PL=sum(PL/10000), PL.percent=sprintf(sum(PL/10000)/sum(Stakes/10000),fmt="%1.4f%%"))
...
ahBets <- data.frame(ddply(mbase2, ~HCap+AHOU+Picked,
                     ...
                     ddply(mbase2, ~HCap+AHOU+Picked, .drop=TRUE, colwise(.fun=length, .cols=~Stakes))[-c(1:3)]) %>% tbl_df
...
*table 3.3.1a* `r llply(split(ahBets,fuBets$AHOU), function(x) paste(unlist(strsplit(as.character(dim(x)),' ')), collapse=' x '))[[1]]`
...
*table 3.3.1b* `r llply(split(ahBets,fuBets$AHOU), function(x) paste(unlist(strsplit(as.character(dim(x)),' ')), collapse=' x '))[[2]]`
...
*table 3.4.1a* `r llply(split(ahBets,ipBets1$AHOU), function(x) paste(unlist(strsplit(as.character(dim(x)),' ')), collapse=' x '))[[1]]`
...
*table 3.4.1b* `r llply(split(ahBets,ipBets1$AHOU), function(x) paste(unlist(strsplit(as.character(dim(x)),' ')), collapse=' x '))[[2]]`
...
#'@ ggplot(mbase2, aes(x = netProbB, y = rEMProbB)) + geom_point(aes(y = netProbB, color = 'netProbB')) + geom_point(aes(y = rEMProbB, color = 'rEMProbB')) + xlab('Bookmaker Backed Net Prob') + ylab('Calculated EM Backed Prob') + theme_economist(base_family='Verdana') + scale_colour_economist() + ggtitle('Bookmaker Net Probabilities -vs- Firm A EM Probabilities')
```
