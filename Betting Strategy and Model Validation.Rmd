...
## 2. Dataset
  ...
  * Section [2.1 Collect and Reprocess the Dataset] - Dataset from Firm A
...
```{r read-data-summary-table, echo=FALSE, cache=TRUE, results='asis'}
## Read the datasets
...
```{r scrap-data, echo=FALSE, cache=TRUE, results='asis'}
...
tmIDdata <- read.csv('datasets/teamID.csv', header=TRUE, sep=',') %>% mutate_each(funs(as.character)) %>% data_frame %>% filter(spbo!='Kuban Krasnodar')
...
## Annual summry graph.
m <- ddply(dat, .(Sess), summarise, Stakes=sum(Stakes), Return=sum(Return), PL=sum(PL)) %>% data_frame
...
## the stakes amount display as $1 = $10,000
m <- ddply(data_frame(Month=as.yearmon(dat$Date,'%b'), dat), .(Month), summarise, Stakes=sum(Stakes), Return=sum(Return), PL=sum(PL)) %>% mutate(Month=as.character(Month)) # mutate Month as.character() at last to make the Month hAxis arrange in order after summarise
...
ahBets <- llply(funs, function(x)
    ddply(dat, ~HCap+AHOU+Picked, .drop=TRUE, colwise(.fun=x, .cols=~Stakes+Return+PL))) %>% join_all(by=c('HCap', 'AHOU', 'Picked')) %>% .[-c((ncol(.)-1):ncol(.))] %>% data_frame
...
## the stakes amount display as $1 = $10,000
lst <- ahBets %>% select(HCap, AHOU, Picked, Stakes, Return, PL) %>% dlply(.(AHOU), gather, Type, Stakes, Stakes:PL) %>% llply(unite, Category, Picked, Type, sep='.') %>% llply(spread, Category, Stakes) %>% llply(data_frame)
...
fuBets <- llply(funs, function(x)
    ddply(dat, ~AHOU+pHKRange+Picked, .drop=TRUE, colwise(.fun=x, .cols=~Stakes+Return+PL))) %>% join_all(by=c('AHOU', 'pHKRange', 'Picked')) %>% .[-c((ncol(.)-1):ncol(.))] %>% date_frame
...
m <- ddply(dat, ~pHKRange+pMYRange, .drop=TRUE, colwise(.fun=sum, .cols=~Stakes+Return+PL)) %>% mutate(R.percent=sprintf(Return/Stakes, fmt='%1.4f%%'), PL.percent=sprintf(PL/Stakes, fmt='%1.4f%%')) %>% arrange(Stakes) %>% tail %>% data_frame
...
## Plot the Stakes and P&L on AH handicap graphs
## the stakes amount display as $1 = $10,000
lst <- fuBets %>% select(AHOU, pHKRange, Picked, Stakes, Return, PL) %>% dlply(.(AHOU), gather, Type, Stakes, Stakes:PL) %>% llply(unite, Category, Picked, Type, sep='.') %>% llply(spread, Category, Stakes) %>% llply(data_frame)
...
ipBets1 <- llply(funs, function(x)
    ddply(dat, ~InPlay2+AHOU+ipRange, .drop=TRUE, colwise(.fun=x, .cols=~Stakes+Return+PL))) %>% join_all(by=c('InPlay2', 'AHOU', 'ipRange')) %>% .[-c((ncol(.)-1):ncol(.))] %>% data_frame
...
lst[[1]] %>% datatable(caption="Table 3.4.1A : Asian Handicap - In-Play Time Range Breakdown ('0,000)", extensions=c('ColReorder', 'ColVis', 'TableTools'), options=list(dom='TC<"clear">rlfrtip', colVis=list(exclude=c(0), activate='mouseover'), tableTools=list(sSwfPath=copySWF(pdf=TRUE)), scrollX=TRUE, scrollCollapse=TRUE))
...
lst[[2]] %>% datatable(caption="Table 3.4.1B : Goal Line - In-Play Time Range Breakdown ('0,000)",extensions=c('ColReorder','ColVis','TableTools'), options=list(dom='TC<"clear">rlfrtip', colVis=list(exclude=c(0), activate='mouseover'), tableTools=list(sSwfPath=copySWF(pdf=TRUE)), scrollX=TRUE, scrollCollapse=TRUE))
```
...
## Plot the Stakes and P&L on AH handicap graphs
## the stakes amount display as $1 = $10,000
lst <- ipBets1 %>% select(InPlay2, AHOU, ipRange, Stakes, Return, PL) %>% dlply(.(AHOU), gather, Type, Stakes, Stakes:PL) %>% llply(unite, Category, InPlay2, Type, sep='.') %>% llply(spread, Category, Stakes) %>% llply(data_frame)
...
ipBets2 <- llply(funs, function(x)
    ddply(dat, ~AHOU+CurScore+ipHCap, .drop=TRUE, colwise(.fun=x, .cols=~Stakes+Return+PL))) %>% join_all(by=c('AHOU', 'CurScore', 'ipHCap')) %>% .[-c((ncol(.)-1):ncol(.))] %>% data_frame
...
lst[[1]] %>% datatable(caption="Table 3.4.2A : Asian Handicap - In-Play Current Score and Handicap Breakdown ('0,000)", extensions=c('ColReorder', 'ColVis', 'TableTools'), options=list(dom='TC<"clear">rlfrtip', colVis=list(exclude=c(0), activate='mouseover'), tableTools=list(sSwfPath=copySWF(pdf=TRUE)), scrollX=TRUE, scrollCollapse=TRUE))
...
lst[[2]] %>% datatable(caption="Table 3.4.2B : Goal Line - In-Play Current Score and Handicap Breakdown ('0,000)", extensions=c('ColReorder', 'ColVis', 'TableTools'), options=list(dom='TC<"clear">rlfrtip', colVis=list(exclude=c(0), activate='mouseover'), tableTools=list(sSwfPath=copySWF(pdf=TRUE)), scrollX=TRUE, scrollCollapse=TRUE))
```
...
## Plot the Stakes and P&L on AH handicap graphs
## the stakes amount display as $1 = $10,000
lst <- ipBets2 %>% select(AHOU, CurScore, ipHCap, Stakes, Return, PL) %>% dlply(.(AHOU), gather, Type, Stakes, Stakes:PL) %>% llply(unite, Category, ipHCap, Type, sep='.') %>% llply(spread, Category, Stakes) %>% llply(data_frame)
...
## 4. Staking Model
  ...
  * Section [4.1 Linear Model] - Analyse the Odds Price and Probabilities
  * Section [4.2 Kelly Model] - aaaaaa
  * Section [4.3 Poisson Modelling] - Soccer Scores Modelling
  * Section [4.4 Staking Modelling and Money Management] - aaaaaa
  * Section [4.5 Expectation Maximization and Staking Simulation] - aaaaaa
...
##
m <- ddply(dat, .(Result), summarise, Stakes=sum(Stakes), Return=sum(Return), Rates=Return/Stakes, n=length(Sess)) %>% data_frame %>% mutate(S.prop=round(Stakes/sum(Stakes), 4), R.prop=round(Return/sum(Return), 4), prop=round(n/sum(n), 4))
