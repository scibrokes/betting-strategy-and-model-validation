...
## the stakes amount display as $1 = $10,000
m <- ddply(data.frame(Month=as.yearmon(dat$Date,'%b'),dat), .(Month), summarise, Stakes=sum(Stakes), Return=sum(Return), PL=sum(PL)) %>% tbl_df %>% mutate(Month=as.character(Month)) # mutate Month as.character at last to make the Month hAxis arrange in order after summarise
...
m <- ddply(dat, ~pHKRange+pMYRange, .drop=TRUE, colwise(.fun=sum, .cols=~Stakes+Return+PL)) %>% mutate(R.percent=sprintf(Return/Stakes,fmt='%1.4f%%'), PL.percent=sprintf(PL/Stakes,fmt='%1.4f%%')) %>% arrange(Stakes) %>% tail %>% tbl_df
...
ipBets2 <- llply(funs, function(x)
    ddply(dat, ~AHOU+CurScore+ipHCap, .drop=TRUE, colwise(.fun=x, .cols=~Stakes+Return+PL))) %>% join_all(by=c('AHOU','CurScore','ipHCap')) %>% .[-c((ncol(.)-1):ncol(.))] %>% tbl_df
...
## Plot the Stakes and P&L on AH handicap graphs
## the stakes amount display as $1 = $10,000
lst <- ipBets2 %>% select(AHOU, CurScore, ipHCap, Stakes, Return, PL) %>% dlply(.(AHOU), gather, Type, Stakes, Stakes:PL) %>% llply(unite, Category, ipHCap, Type, sep='.') %>% llply(spread, Category, Stakes) %>% llply(tbl_df)
...
dat %>% select(No,EUPrice,HKPrice,fHKPriceL,fMYPriceB,fMYPriceL,netProbB,netProbL,rEMProbB,rEMProbL,favNetProb,undNetProb) %>% .[sample(1:nrow(.),6),] %>% tbl_df %>% kable(.,caption='Table 4.1.2 : Probabilities Table')
```
