---
title: "Betting Strategy and Model Validation (テスト)"
...
<iframe width="420" height="315" src="https://www.youtube.com/embed/V_5O0GUEPo4" frameborder="0" allowfullscreen></iframe>
...
tmIDdata <- read.csv('datasets/teamID.csv', header = TRUE, sep = ',') %>% mutate_each(funs(as.character)) %>% data.frame %>% tbl_df %>% filter(spbo != 'Kuban Krasnodar')
...
m <- ddply(dat, ~ HCap + AHOU, .drop = TRUE, colwise(.fun = sum, .cols = ~ Stakes + Return + PL)) %>% mutate(R.percent = sprintf(Return/Stakes, fmt = '%1.4f%%'), PL.percent = sprintf(PL/Stakes, fmt = '%1.4f%%')) %>% dlply(.(AHOU)) %>% llply(arrange, Stakes) %>% ldply(tail, 4, .id = NULL)
m %>% kable(caption = 'Table 3.2.1c : Sample data about Handicap, Stakes and PL')
```

*table 3.2.1c* : `r paste0(dim(m), collapse = ' x ')` : *Handicap, stakes and PL sample table.*
...
lst[[2]] %>% datatable(
  caption = "Table 3.3.1B : Goal Line - Price Range Breakdown ", 
  escape = FALSE, filter = "top", rownames = FALSE, 
  extensions = list("ColReorder" = NULL, "RowReorder" = NULL, 
                    "Buttons" = NULL, "Responsive" = NULL, 
                    "FixedColumns" = list(leftColumns = 1)), 
...
## 4. Staking Model
  ...
  * Section [4.2 Linear Model] - 
  * Section [4.3 Kelly Model] - aaaaaa
  * Section [4.4 Poisson Modelling] - Soccer Scores Modelling
  * Section [4.5 Staking Modelling and Money Management] - aaaaaa
  * Section [4.6 Expectation Maximization and Staking Simulation] - aaaaaa
...
gvis.options <- list(title = "Bookmaker Net Probabilities -vs- Firm A EM Probabilities", series = "[{targetAxisIndex:0},{targetAxisIndex:1}]", hAxis = "{title:'Bookmaker Backed Net Prob'}", vAxis = "{title:'rEMProbB'},{title:'netProbB'}", width = 'automatic', height = 'automatic')

line.gvis <- gvisLineChart(xvar = 'netProbB', yvar = 'rEMProbB', data = dat, options = gvis.options)
plot(line.gvis)
...
### 4.2 Linear Model
  ...
  Again, I don't pretend to know the correct model, here I try to apply linear model, and descrete linear model as well as bayesian etc. Kindly refer to [Staking Linear ®egression Ⓜodels](http://rpubs.com/englianhu/staking-linear-regression-models).
...
####  Reference for industry knowdelege and academic research portion for the paper.
  ...
  1) [ () - *Creating a Profitable Betting Strategy for Football by Using Statistical Modelling*](http://www.ylikerroin.com/file/Complete.pdf)
  2) [ () - *What Actually Wins Soccer Matches: Prediction of the 2011-2012 Premier League for Fun and Profit*](http://homes.cs.washington.edu/~jasnyder/papers/thesis.pdf)
  3) [ () - *The value of statistical forecasts in the UK association football betting market*](http://www.sciencedirect.com/science/article/pii/S016920700400010X)
  4) [ () - *Dixon and Cole's Poisson regression R Packages*](http://lastplanetranking.blogspot.com/2013/11/code.html)
  5) [ () - *Apply Kelly-Criterion on English Soccer 2011/2012*](http://rpubs.com/englianhu/kelly_eng1112) and [ () - *Apply Kelly-Criterion on English Soccer 2012/2013*](http://rpubs.com/englianhu/kelly_eng1213)
  6) [ () - *An introduction to football modelling at Smartodds*](https://people.maths.ox.ac.uk/siamstudentchapter/webpages/2011_Conference/Robert_Talk.pdf)
  7) [ () - *The Betting Machine*](http://www.diva-portal.org/smash/get/diva2:655630/fulltext01.pdf)
  8) [ () - *The Kelly Criterion in Blackjack Sports Betting, and the Stock Market*](http://www.eecs.harvard.edu/cs286r/courses/fall12/papers/Thorpe_KellyCriterion2007.pdf)
  9) [Fabián Enrique Moya (2012) - *Statistical Methodology for Profitable Sports Gambling*](https://www.stat.sfu.ca/content/dam/sfu/stat/alumnitheses/2012/FabianMoyaFinalVersion.pdf)
  10) [ () - *How to apply the Kelly criterion when expected return may be negative?*](http://quant.stackexchange.com/questions/2500/how-to-apply-the-kelly-criterion-when-expected-return-may-be-negative)
  11) [ () - *Money Management Using The Kelly Criterion*](http://www.investopedia.com/articles/trading/04/091504.asp)
  12) [ () - *Optimal Exchange Betting Strategy For WIN-DRAW-LOSS Markets*](http://www.rankingsoftware.com/research/OptimalBettingStrategyWinDrawLoss.pdf)
  13) [ () - *Kelly criterion with more than two outcomes*](http://math.stackexchange.com/questions/662104/kelly-criterion-with-more-than-two-outcomes)
  14) [ () - *凯利模式资金管理*](http://ch-hsieh.blogspot.com/2015/01/kelly-criterion-0.html)
  15) [John Fingleton & Patrick Waldron (1999) - *Optimal Determination of Bookmakers' Betting Odds: Theory and Tests*](http://www.maths.tcd.ie/~pwaldron/pdf/tp969.pdf)
  16) [Maurizio Montone (2015) - *Optimal Pricing in the Online Betting Market*](http://www.efmaefm.org/0EFMAMEETINGS/EFMA%20ANNUAL%20MEETINGS/2015-Amsterdam/papers/EFMA2015_0502_fullpaper.pdf)
  17) [Steven D. Levitt (2004) - *Why are Gambling Markets Organised so Differently from Financial Markets?*](https://www.stat.berkeley.edu/~aldous/157/Papers/Levitt_Gambling_2004.pdf)
  18) [Steven Xu (2013) - *Forecasting Accuracy and Line Changes in the NFL and College Football Betting Markets*](http://blogs.colgate.edu/economics/files/2013/05/Xu_Econ490_Thesis.pdf)
  19) [Kwinten Derave (2013-2014) - *The Forecast Ability of the Dispersion of Bookmaker Odds*](http://lib.ugent.be/fulltxt/RUG01/002/165/010/RUG01-002165010_2014_0001_AC.pdf)

####  Reference for technical research on programming and coding portion for the paper.

  17) [ () - *Wrangling F1 Data With R*](http://www.r-bloggers.com/wrangling-f1-data-with-r-f1datajunkie-book/)
  18) [ () - *Interactive visualizations with R - a minireview*](http://ouzor.github.io/blog/2014/11/21/interactive-visualizations.html)
  19) [ () - *R + htmlwidgets + DT + sparkline*](https://englianhu.wordpress.com/2015/10/04/r-htmlwidgets-dt-sparkline/)
