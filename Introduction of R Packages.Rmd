...
suppressAll(requirePackages(c('plyr', 'magrittr', 'stringr', 'tidyverse', 'formattable', 'doParallel')))
...
```{r setwd, echo = FALSE, results = 'asis'}
pth <- 'C:/Users/scibr/Documents/GitHub/scibrokes/betting-strategy-and-model-validation'
```

```{r get-data-summary-table-2.1, warning = FALSE}
nrow(do.call(rbind, llply(as.list(seq(2011, 2015)), function(x) data.frame(Sess = x, read.csv(paste0(pth, '/data/', x, '.csv'))), .parallel = FALSE)))

nrow(bind_rows(llply(as.list(seq(2011, 2015)), function(x) data.frame(Sess = x, read.csv(paste0(pth, '/data/', x, '.csv'))), .parallel = FALSE)))

system.time(do.call(rbind, llply(as.list(seq(2011,2015)), function(x) data.frame(Sess = x, read.csv(paste0(pth, '/data/', x, '.csv'))), .parallel = FALSE)))

system.time(bind_rows(llply(as.list(seq(2011, 2015)), function(x) data.frame(Sess = x, read.csv(paste0(pth, '/data/', x, '.csv'))), .parallel = FALSE)))

system.time(bind_rows(llply(as.list(seq(2011, 2015)), function(x) data.frame(Sess = x, data.table::fread(paste0(pth, '/data/', x, '.csv'))), .parallel = FALSE)))
```
...
```{r bank-roll1B, eval = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
...
## Exactly same with *graph 4.3.5.1* from http://rpubs.com/englianhu/208636. Working fine in rmarkdown but unable work in rmarkdown-shiny, here I set `eval = FALSE`

#Quitting from lines 260-269 (Introduction_of_R_Packages.Rmd) 
#
#Warning: Error in order: 嶲悢1晄惀栴検
...
*table 2.5.1 : Price flutuation table.*
  ...
  Here are some examples about applicatino of highchart on shiny. The original link is <http://rpubs.com/englianhu/introduction-of-r-packages> while I added shinyapp and post as shiny rmarkdown file.
