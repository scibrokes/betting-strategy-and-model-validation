...
# 4. Staking Ⓜodel
  ...
  - Section [4.5 Staking Ⓜodel and Ⓜoney Ⓜanagement] - Simulate the staking model. and fund portfolio management
    + Section [4.5.1 Risk Management] - Introduce to Investment Fund Management
    + Section [4.5.2 Markowitz Portfolio] - Optimal Investment Fund Portfolio
    + Section [4.5.3 Optimal Kelly Portfolio]
    + Section [4.5.4 Investors' Fund Refill]
...
m <- ddply(dat, .(Sess), summarise, Stakes = sum(Stakes), PL = sum(PL), n = length(Sess), rRates = PL / Stakes) %>% mutate(Stakes = currency(Stakes), PL = currency(PL), n = accounting(n, format = 'd'), rRates = formattable::percent(rRates)) %>% tbl_df
...
tagList(
  ...
  as.htmlwidget(m %>% formattable(list(
    ...
    PL = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'green', 'gray')), x ~ paste0(round(x, 2), ' (rank: ', sprintf('%02d', rank(-x)), ')')), 
    
    n = color_tile('white', '#9B870C'), 
    
    rRates = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'green', 'gray')), x ~ sprintf('%1.2f%% (rank: %02d)', 100 * x, rank(-x)))))))
```
...
m1 %<>% purrr::map_if(is.character, as.numeric) %>% data.frame %>% tbl_df %>% mutate(Stakes = currency(Stakes), Return = currency(Return), Rates = formattable::percent(Rates), n = accounting(n, format = 'd'), S.prop = formattable::percent(S.prop), R.prop = formattable::percent(R.prop), prop = formattable::percent(prop))
...
tagList(
  ...
  as.htmlwidget(m1 %>% formattable(list(
    ...
    #'@ Stakes = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'blue', 'grey')), x ~ sprintf('%.2f (rank: %.0f)', x, rank(-x))),
    Stakes = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'blue', 'grey')), x ~ paste0(round(x, 2), ' (rank: ', sprintf('%02d', rank(-x)), ')')), 
    
    #'@ Return = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'blue', 'grey')), x ~ sprintf('%.2f (rank: %.0f)', x, rank(-x))),
    Return = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'blue', 'grey')), x ~ paste0(round(x, 2), ' (rank: ', sprintf('%02d', rank(-x)), ')')), 
    
    Rates = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'blue', 'grey')), x ~ sprintf('%1.2f%% (rank: %.0f)', 100 * x, rank(-x))),
    
    n = color_tile('white', 'green'),
    
    S.prop = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'blue', 'grey')), x ~ sprintf('%1.2f%% (rank: %2d)', 100 * x, rank(-x))),
    
    R.prop = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'blue', 'grey')), x ~ sprintf('%1.2f%% (rank: %.0f)', 100 * x, rank(-x))),
    
    prop = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'blue', 'grey')), x ~ sprintf('%1.2f%% (rank: %.0f)', 100 * x, rank(-x)))
))))
```
...
dat[c('No.x', 'EUPrice', 'HKPrice', 'fHKPriceL', 'fMYPriceB', 'fMYPriceL', 'netProbB', 'netProbL', 'rRates', 'rEMProbB', 'rEMProbL', 'netEMEdge', 'favNetProb', 'undNetProb')] %>% .[sample(1:nrow(.), 6), ] %>% formattable %>% as.htmlwidget
```
...
hchart(dat[c('netProbB', 'rEMProbB', 'pMYRange')], 'scatter', hcaes(x = netProbB, y = rEMProbB, group = pMYRange))
```
...
hdp %>% formattable(list(
  X1 = formatter('span', style = x ~ formattable::style(color = ifelse(x > 0, 'green', 'red')), x ~ icontext(ifelse(x > 0, x, -x), ifelse(x, x, x))), 
  X2 = formatter('span', style = x ~ formattable::style(color = ifelse(x > 0, 'green', 'red')), x ~ icontext(ifelse(x > 0, x, -x), ifelse(x, x, x))), 
  X3 = formatter('span', style = x ~ formattable::style(color = ifelse(x > 0, 'green', 'red')), x ~ icontext(ifelse(x > 0, x, -x), ifelse(x, x, x))), 
  X4 = formatter('span', style = x ~ formattable::style(color = ifelse(x > 0, 'green', 'red')), x ~ icontext(ifelse(x > 0, x, -x), ifelse(x, x, x))), 
  X5 = formatter('span', style = x ~ formattable::style(color = ifelse(x > 0, 'green', 'red')), x ~ icontext(ifelse(x > 0, x, -x), ifelse(x, x, x))), 
  X6 = formatter('span', style = x ~ formattable::style(color = ifelse(x > 0, 'green', 'red')), x ~ icontext(ifelse(x > 0, x, -x), ifelse(x, x, x))) 
  )
) %>% as.htmlwidget
```
...
bestlm <- compare %>% filter(p.value < 0.05) %>% group_by(Cat) %>% slice(which.min(BIC)) %>% mutate(AIC = accounting(AIC), BIC = accounting(BIC), df = accounting(df, format = 'd'), residuals = accounting(residuals, format = 'd'), p.value = formattable::percent(p.value), delta.AIC = accounting(delta.AIC), delta.BIC = accounting(delta.BIC)) #best model for every single category.

compare %<>% mutate(AIC = accounting(AIC), BIC = accounting(BIC), df = accounting(df, format = 'd'), residuals = accounting(residuals, format = 'd'), p.value = formattable::percent(p.value), delta.AIC = accounting(delta.AIC), delta.BIC = accounting(delta.BIC)) %>% split(., .$Cat) #split the category and list the ranking.
...
m <- ddply(dat, .(Sess), summarise, Stakes = sum(Stakes), PL = sum(PL), n = length(Sess), rRates = PL / Stakes) %>% mutate(Stakes = currency(Stakes), PL = currency(PL), rRates = formattable::percent(rRates)) %>% tbl_df

m %>% formattable(list(

  Stakes = color_tile('white', 'darkgoldenrod'),
  Return = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'green', 'gray')), x ~ sprintf('%.2f (rank: %02d)', x, rank(-x))),
  n = color_tile('white', '#9B870C'),
  rRates = formatter('span', style = x ~ formattable::style(color = ifelse(rank(-x) <= 3, 'green', 'gray')), x ~ sprintf('%.4f (rank: %02d)', x, rank(-x)))
)) %>% as.htmlwidget
```
...
#'@   }) %>% bind_rows %>% mutate(sm = seq(nrow(.))) %>% ddply(.(Category), summarise, Stakes = mean(Stakes), Return = mean(Return), PL = mean(PL), PL.R = PL / Stakes) %>% mutate(Stakes = currency(Stakes), Return = currency(Return), PL = currency(Return), PL.R = formattable::percent(PL.R))
...
#'@ }) %>% bind_rows %>% mutate(sm = seq(nrow(.))) %>% mutate(sm = seq(nrow(.))) %>% ddply(.(Category), summarise, Stakes = mean(Stakes), Return = mean(Return), PL = mean(PL), PL.R = PL / Stakes) %>% mutate(Stakes = currency(Stakes), Return = currency(Return), PL = currency(Return), PL.R = formattable::percent(PL.R))
...
theta <- ddply(dat2, .(Sess), summarise, rRates = formattable::percent(mean(rRates)), 
...
tagList(
  ...
  as.htmlwidget(K1D1sum %>% formattable(list(
    ...
    mse = formatter('span', style = x ~ formattable::style(color = ifelse(rank(x) <= 3, 'blue', 'grey')), x ~ sprintf('%.6f (rank: %.0f)', x, rank(x)))))))
```
...
```{r bank-roll-portfolio, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
## remove all previous objects except 'op' (googleVis ggvis.option) to save memory.
## http://stackoverflow.com/questions/6190051/how-can-i-remove-all-objects-but-one-from-the-workspace-in-r/6190319
rm(list = setdiff(ls(), 'op'))
...
<br>
  ...
  Above table shows the return rates of investment fund from firm A via agent A. We know the initial unvestment fund from `r currency(as.numeric(Op(SOFund)[1,])) * 10000` growth to `r currency(as.numeric(tail(Cl(SOFund), 1))) * 10000` within year `r row.names(data.frame(SOFund)) %>% range` with a high return rates `r formattable::percent(as.numeric(tail(Cl(SOFund), 1) / as.numeric(Op(SOFund)[1,])))`. Well, I try to apply Kelly model for stakes management which is in previous section [4.3 Kelly Ⓜodel].

  From *table 4.3.5.2* we can know the risk of the all investmental funds. You are feel free to browse over [KellyApps](https://beta.rstudioconnect.com/content/2311/)^[*shinyapp 4.3.5.1*] for more details.
  ...
  I had built and test `r list.files('./data', pattern = '^K.+\\.rds$') %>% length` Kelly main funds (split to indipedent fund will be altogether `r list.files('./KellyApps/data1', pattern = '^K.+\\.rds$') %>% length %>% formattable::digits(., 3, format = 'd', big.mark = ',')` funds) but now need to set a baseline for every single leagues and simulate again the steps in [4.3 Kelly Ⓜodel] to made a comparison based on the portion of stakes from the initial pools for every single league as we can refer to *table 4.3.2.1*.

```{r fund-reallocation, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
## remove object to save memory.
if(exists('SOFund')) rm(SOFund)
...
## After we set a united initial fund size and set a baseline and portfolio for staking on every single league, here we need to test the Kelly.
## 
## need to load again the saved raw data and start from 0 again with mentioned criterias.
#'@ load('./regressionApps/shinyData.RData', envir = .GlobalEnv)
...
lRiskSettingProf <- lRProf1A %>% 
  mutate(portion = formattable::percent(all.lg/sum(all.lg)))

## fit the stakes based on the league portion among the fund.
#'@ dat <- join(dat, lRiskSettingProf) %>% tbl_df %>% mutate(League = factor(League))
#'@ dat %<>% mutate(Stakes = Stakes / mean(Stakes) * baseline)
#'@ dat[c('all.lg', 'portion', 'baseline')] <- NULL

## 
## After run the chunk `fund-reallocation`, we need to use below functions all recalculate the Kelly models in chunk `resimulateKelly`.
## ================================================================

ttd <- lRiskSettingProf %>% .[-1] %>% colSums %>% t %>% 
  data.frame(League = 'Total', .) %>% tbl_df %>% 
  mutate(portion = formattable::percent(portion))
...
tagList(
  ...
  as.htmlwidget(lRiskSettingProf %>% .[sample(1:nrow(.), 6), ] %>% rbind(., ttd)%>% formattable(list(
...
```{r resimulateKelly, eval = FALSE, echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
## -------------------- Resimulate Kelly models --------------------
## Similar with profHandling() for load or save profiles at once. You can just using below function to load all Kelly models, includes save or load.
## 
## Due to direct calculate lRProf and wProf in vKelly() and vKelly2() will be faced errors, here I saved above and directly call and fit into calculation as below KModelslite().

#'@ source('./function/KModels.R', local = TRUE)
#'@ KMods <- KModels(action = 'save')

## A lite version function of KModels() due to size allocation issues.
#'@ source('./function/KModelslite.R', local = TRUE)
#'@ KModslite <- KModelslite(action = 'save', .print = TRUE)

## save every funds independently in order to easier load/read.
#'@ source('./function/splitFund.R', local = TRUE)
#'@ splitFund(pth = 'portfolio', .print = TRUE, parallel = TRUE, progress = 'text')
...
![*equation 4.5.4.1 : Economic Order Quantity (EOQ)*](./figure/EOQ.jpg)
...
### 4.6.1 Truncated Bivariate Normal Distribution
  ...
  Before I simulate all Kelly funds by resampling the scores, here I using few different *trancated bivariate normal distribution*^[You can refer to few r packages which are [MASS](https://cran.r-project.org/web/packages/MASS/index.html), [mvtnorm](https://cran.r-project.org/web/packages/mvtnorm/index.html) and [tmvtnorm](https://cran.r-project.org/web/packages/tmvtnorm/index.html).] and the weight parameters to get the optimal randomise scores. [tmvtnorm - A Package for the Truncated Multivariate Normal Distribution]()
...
# dat[c('FTHG', 'FTAG')]
lms$opt0 <- lm(FTHG~FTAG, data = scoresall$data)
```
...
compare %<>% mutate(AIC = accounting(AIC), BIC = accounting(BIC), df = accounting(df, format = 'd'), residuals = accounting(residuals, format = 'd'), p.value = formattable::percent(p.value), delta.AIC = accounting(delta.AIC), delta.BIC = accounting(delta.BIC))
```
...
*table 5.1.1.1* : `r paste0(dim(BRSum), collapse = ' x ')` : *Comparison of Kelly investmemt fund.*

```{r csv}
BRSum.Op <- data.frame(BRSum[c(1:5)], Op(BRSum)) %>% tbl_df
write.csv(BRSum.Op, './data/BRSum.Op.csv')

BRSum.Hi <- data.frame(BRSum[c(1:5)], Hi(BRSum)) %>% tbl_df
write.csv(BRSum.Hi, './data/BRSum.Hi.csv')

BRSum.Lo <- data.frame(BRSum[c(1:5)], Lo(BRSum)) %>% tbl_df
write.csv(BRSum.Lo, './data/BRSum.Lo.csv')

BRSum.Cl <- data.frame(BRSum[c(1:5)], Cl(BRSum)) %>% tbl_df
write.csv(BRSum.Cl, './data/BRSum.Cl.csv')

BRSum.Vo <- data.frame(BRSum[c(1:5)], Vo(BRSum)) %>% tbl_df
write.csv(BRSum.Vo, './data/BRSum.Vo.csv')

BRSum.Ad <- data.frame(BRSum[c(1:5)], Ad(BRSum)) %>% tbl_df
write.csv(BRSum.Ad, './data/BRSum.Ad.csv')
```

  Due to the file is too big, you are feel free to download via [BRSum]() or seperate files as below.
...
## 5.2 Market Basket
  ...
  - [arules — Mining Association Rules and Frequent Itemsets with R](https://cran.r-project.org/web/packages/arules/README.html)
  - [Market basket analysis](https://rpubs.com/sbushmanov/180410)

```{r arules, eval = FALSE, include = FALSE}
library('arules')
library('arulesViz')
library('visNetwork')
library('igraph')
...
if(require('shinyjs', quietly = TRUE)) detach('package:shinyjs') ##https://github.com/daattali/shinyjs/issues/41#issuecomment-336700247

source('./function/readfirmData.R')
source('./function/arrfirmData.R')
...
mbase <- suppressAll(readfirmData(years = years, pth = './data/') %>% arrfirmData(lProfile = lProfile))
mbase <- mbase$datasets

mbase <- mbase %>% unite(matchID, Home, Away, sep ='.') %>% unite(matchID, DateUK, matchID, sep ='.')
mbase <- mbase[c('matchID', 'Selection')]

## https://stackoverflow.com/questions/17313450/how-to-convert-data-frame-to-transactions-for-arules?answertab=votes#tab-top
## https://stackoverflow.com/questions/44618956/convert-r-data-frame-column-to-arules-transactions?answertab=votes#tab-top
#'@ items <- strsplit(as.character(mbase$Selection), ', ')
items <- llply(split(mbase, mbase$matchID), function(x) as.character(x$Selection))

trans3 <- as(items, 'transactions')
saveRDS(trans3, './data/trans3.rds')
```
```{r read-arules, warning = FALSE, echo = FALSE}
library('arules')
library('arulesViz')
library('visNetwork')
library('igraph')
...
trans3 <- readRDS('./data/trans3.rds')
```

```{r arules1, echo = FALSE}
par(mfrow=c(1,2))

itemFrequencyPlot(trans3,
                  type="relative",
...
```{r arules2, echo = FALSE}
par(mar=c(2,10,2,2), mfrow=c(1,2))

barplot(sort(table(unlist(LIST(trans3))))[1:10]/9835,
        horiz=TRUE,
        las=1,
        col='steelblue3',
        xlab='',
...
barplot(sort(table(unlist(LIST(trans3))))[1:10],
        horiz=TRUE,
        las=1,
        col='steelblue3',
        xlab='',
...
```{r arules3, echo = FALSE}
tbl <- crossTable(trans3)
tbl[1:5, 1:5]
```
...
```{r arules4, echo = FALSE}
crossTable(trans3, measure = 'lift', sort = TRUE)[1:5,1:5]
```
...
![**Terminator II**](figure/Ill-Be-Back-Terminator.gif)
  ...
  - [Convert R data.frame column to Arules transactions](https://stackoverflow.com/questions/44618956/convert-r-data-frame-column-to-arules-transactions?answertab=votes#tab-top)

  - [how to convert data.frame to transactions for arules](https://stackoverflow.com/questions/17313450/how-to-convert-data-frame-to-transactions-for-arules?answertab=votes#tab-top)
