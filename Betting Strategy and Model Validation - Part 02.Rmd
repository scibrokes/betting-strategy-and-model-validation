...
*table 4.2.1 : Best model to test the efficiency of staking.*
```
  Base on above summary, we can decide that the model *`r filter(compare, BIC == min(BIC), AIC == min(AIC)) %>% .$.id`*^[BIC will be primary reference while AIC is the secondary reference. The smallest value is the best model.] is the best fit to determine the factors and effects to place stakes for all matches^[mixed InPlay + Pre-match, all observations or soccer matches placed betsare `r nrow(dat)`.]. The timing of InPlay and the stakes amount is the major effects to the return of investment.
```
