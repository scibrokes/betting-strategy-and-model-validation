...
# 4. Staking Ⓜodel
  ...
  - Section [4.3 Kelly Ⓜodel] - Test the Kelly Model.
    ...
    + Section [4.3.2 Weight Function] - Modified Kelly model to increase the effeciency of staking and profit.
...
m <- ddply(dat, .(Sess), summarise, Stakes = sum(Stakes), PL = sum(PL), n = length(Sess), rRates = PL / Stakes) %>% mutate(Stakes = accounting(Stakes), PL = accounting(PL), rRates = percent(rRates))

m %>% formattable(list(
  caption = 'Table 4.1.1 : Annual Return of Investment (HKD$1 = HKD$10,000)',
...
*table 4.1.1* : `r paste0(dim(m), collapse=' x ')` : *Return of annually investment summary table. (HKD$1 indicates HKD$10,000)*^[Kindly refer to the list of colors via [Dark yellow with hexadecimal color code #9B870C](http://encycolorpedia.com/9b870c) for plot the stylist table.]
...
m1 %<>% purrr::map_if(is.character, as.numeric) %>% data.frame %>% tbl_df
#'@ m %>% kable(caption = 'Table 4.1.3 : Summary of Betting Results')

m1 %>% formattable(list(
    caption = 'Table 4.1.2 : Summary of Betting Results (HKD$1 = HKD$10,000)',
...
*table 4.1.2* : `r paste0(dim(m1), collapse = ' x ')` : *Summary of betting results. (HKD$1 = HKD$10,000)*
...
dat$rRates <- rep(m$rRates, count(dat, Sess)$n)
dat$rEMProbB = round(dat$rRates * dat$netProbB, 6)
dat$rEMProbL = round(1- dat$rEMProbB, 6)

dat %>% select(No.x, EUPrice, HKPrice, fHKPriceL, fMYPriceB, fMYPriceL, netProbB, netProbL, rRates, rEMProbB, rEMProbL, favNetProb, undNetProb) %>% .[sample(1:nrow(.), 6), ] %>% formattable %>% as.htmlwidget
```

*table 4.1.3* : `r paste0(dim(dat), collapse = ' x ')` : *Odds price and probabilities sample table. (HKD$1 = HKD$10,000)*
...
compare[[1]] %>% formattable(list(
  ...
  p.value = formatter('span', style = x ~ formattable::style(color = ifelse(rank(x) <= 3, 'blue', 'grey')), x ~ sprintf('%.4f (rank: %02d)', x, rank(x))),
...
bestlm %>% formattable(list(
  ...
  p.value = formatter('span', style = x ~ formattable::style(color = ifelse(rank(x) <= 3, 'blue', 'grey')), x ~ sprintf('%.4f (rank: %.0f)', x, rank(x))),
...
$$S = \frac{\rho_{EM} \times BK_{Decimal\ odds} - 1} {BK_{HK\ odds}} \times BR \cdots equation 4.3.1$$

  - Where $S$ is the stake expressed as a fraction of one's total bankroll, $BR$ is the total bankroll.
...
m %>% formattable(list(
  caption = 'Table 4.3.1 : Annual Return of Investment without Cancelled Bets (HKD$1 = HKD$10,000)',
...
*table 4.3.1* : `r paste0(dim(m), collapse=' x ')` : *Return of annually investment summary table without cancelled bets. (HKD$1 = HKD$10,000)*^[the `rRates` is the mean value of annual return rates which is the return divides by stakes but ommit the cancelled/voided bets to avoind the bias.]

  The `rRates` value from table above excludes the `Cancelled` bets. By refer to *equation 4.3.1*, now we fit the adge value from *equation 4.1.1* into it to get the `rEMProbB2` and `rEMProbL2` with known staked value $S$^[Although the result will not be accurate due to the we mention at first, the firm A will not only place bets via only agent A. Let say edge of 0.10 and 0.20 also placed maximum bet HKD40000 but the firm A might placed different amount through other agency based on different edge. However based on the stakes we can reverse the optimal EM Odds.] to replace the existing EM value.
</s>

$$\rho_{EM} = \frac {S \times BK_{HK\ odds} + 1} {BK_{Decimal\ odds}} \cdots equation4.3.2$$
...
BR <- ddply(dat, .(DateUS), summarise, Stakes = sum(Stakes), Return = sum(Return), PL = sum(PL), n = length(Sess), rRates = Return / Stakes) %>% mutate(CumStakes = cumsum(Stakes), SPL = cumsum(PL), BR = ifelse(4600 + SPL > 0, 4600 + SPL, -9999), gRates = c(1, exp(diff(log(BR$BR), lag = 1))), gRates2 = BR/BR[1])
...
dat %<>% mutate(KStakes = (sum(Stakes) * HKPrice + 1) / EUPrice, KReturn = ifelse(Result == 'Win', KStakes * EUPrice, ifelse(Result == 'Half Win', KStakes * (HKPrice * 0.5 + 1), ifelse(Result == 'Push', KStakes, ifelse(Result == 'Half Loss', KStakes * 0.5, ifelse(Result == 'Loss', 0, NA))))))
...
K <- llply(split(dat, dat$Result), function(x) x[sample(nrow(x), 2), c('Result', 'KReturn', 'Return', 'EUPrice', 'HKPrice', 'KStakes', 'Stakes', 'Rebates')]) %>% ldply(., .id = 'Result') %>% mutate(KReturn = accounting(KReturn), Return = accounting(Return), KStakes = accounting(KStakes), Stakes = accounting(Stakes), Rebates = accounting(Rebates), Change = percent(KStakes / Stakes - 1)) %>% .[sample(nrow(.), nrow(.)), ]
...
*table 4.3.3* : `r paste0(dim(K), collapse=' x ')` : *Kelly stakes and firm A's stakes of annually investment summary table without cancelled bets.*^[The annum return rates but not all data return rates since we need to use the return of that particular year to reverse. The weight parameter in later section will take last year performance to be weight controller. Due to the firm A does not only placed bets with one agent but couples of agencies. Let say placed one millions HKD on a soccer match with higher price simultaniously compare to placed bets on one agency with price decreasingly, therefore the Kelly stakes will definately defferent with firm A's stakes.]

  Above Kelly stakes table 

```{r}
ddply(K, .(Result), summarise, 'KReturn (in 0,000)' = sum(KReturn), 'Return (in 0,000)' = sum(Return), EUPrice = mean(EUPrice), HKPrice = mean(HKPrice), 'KStakes (in 0,000)' = sum(KStakes), 'Stakes (in 0,000)' = sum(Stakes), Rebates = sum(Rebates), Change = mean(Change)) %>% tbl_df %>% formattable %>% as.htmlwidget
```

```
## comparison among the Kelly models and the existing staking model by firm A.
> dat %>% mutate(KStakes = exp((log(1000) * HKPrice + 1) / EUPrice), KReturn = ifelse(Result == 'Win', KStakes * EUPrice, ifelse(Result == 'Half Win', KStakes * (HKPrice * 0.5 + 1), ifelse(Result == 'Push', KStakes, ifelse(Result == 'Half Loss', KStakes * 0.5, ifelse(Result == 'Loss', 0, NA)))))) %>% select(HKPrice, EUPrice, Stakes, Result, KStakes, Return, KReturn) %>% .[-4] %>% colSums
   HKPrice    EUPrice     Stakes    KStakes     Return    KReturn 
  38740.58   79773.69 1662571.23 2000310.10 1698343.56 2064908.17 
> 2064908.17/2000310.10
[1] 1.032294
> 1698343.56/1662571.23
[1] 1.021516
```
...
$$S_N = (1 + K)^W(1 - K)^L S_0 \cdots equation 4.3.3$$
...
$$L(\rho) = \prod_{i=1}^{n} (x_{i}|\rho) \cdots equation 4.3.4$$
  ...
  Part I inside *®γσ, Eng Lian Hu (2014)*^[Kindly refer to 4th paper in [Reference for industry knowdelege and academic research portion for the paper.] under **7.4 References**] has applied it and generates an impressive return.
...
### 4.3.2 Weight Function
  ...
  Similar with my prevous Rmodel which applied on Poisson model. Due to the calculation of the settlement and result on the win and loss of Asian Handicap is different with Fixed odds, the probabilities of the outcome will be descrete and the measurement of the `likelihood` result required in order to maximize the profit. here we need to add an additional parameter controller to adjust the staking amount on every single match.

  Now I try to will simulate an enhanced Kelly model on staking which take the effect of the outcome of the result into calculation.

$$\phi(r) = exp(-rX) \cdots equation 4.3.5$$
...
=& Push\\
=& Half\ Loss\\
=& Loss\end{Bmatrix} \cdots equation 4.3.6$$
...
## 7.3 Speech and Blooper
  ...
  - **The variable `Sess` and `Month` SUDDENTLY auto added when I select `Day` inside chunk `data-daily-summary` in Part I. solved by changed the class `groups_df` to be `tbl_df` by `%>% tbl_df`. The Viewer panel inside RStudio forced to open diagonal (similar with Selangor flag) with source codes otherwise will suto shutdown apps. Now I know that is wizard control it. Again, I am not politician. Wonder why the wizard (HuaCai being Kuala Selangor No32 Tmn RHU) known he will be end with death but keep alive to killing human and casue endless sorrow (He used to say all companies are same company solely because of he one person don't want to die to make millions human died innocently). There was two flights in Shanghai almost crash by accident and more than 400 passengers will die just due to the wizard control the human besides me to be dummies. I went to Selangor Majlis Petaling Jaya Library yesterday and reading but the people inside library all became zombies and the AirAsia cancelled 27~40 flights.**
