...
compareKelly(K1, chart = TRUE, type = 'multiple')
```
...
*table 4.5.1* : `r paste0(dim(K), collapse = ' x ')` : *Kelly stakes and firm A's stakes of annually investment summary table.*
...
$$L(\rho) = \prod_{i=1}^{n} (x_{i}|\rho) \cdots equation\ 4.3.4$$
  ...
  Now I try to simulate all 110 Kelly weighted models 100 times by resampling the scores by using *trancated bivariate normal distribution*^[You can refer to few r packages which are [MASS](https://cran.r-project.org/web/packages/MASS/index.html), [mvtnorm](https://cran.r-project.org/web/packages/mvtnorm/index.html) and [tmvtnorm](https://cran.r-project.org/web/packages/tmvtnorm/index.html).] and the weight parameters to get the optimal result.
