...
## Run codes in app.R inside the folder `regressionApps` and save.images() and now directly load for shinyApp use.
...
```{r filtered-data, echo = FALSE, results = 'asis'}
...
dat %>% select(No, EUPrice, HKPrice, fHKPriceL, fMYPriceB, fMYPriceL, netProbB, netProbL, rRates, rEMProbB, rEMProbL, favNetProb, undNetProb) %>% .[sample(1:nrow(.), 6), ] %>% formattable %>% as.htmlwidget
```
...
suppressMessages(require('highcharter', quietly = TRUE))
...
hdp %<>% matrix(., nrow = 7) %>% data.frame %>% tbl_df
...
$$S = \frac{(\rho_{win} \times BK_{Decimal\ odds} - \rho_{lose})} {(BK_{Decimal\ odds})} \cdots equation 4.3.1$$

  - Where $S$ is the stake expressed as a fraction of one's total bankroll,
  - $\rho_{win}$ is probability of an event to take place and 
  - $\rho_{lose} is probability of lose$ 
  - while $BK_{Decimal\ odds}$ is decimal odds (decimal Odds with capital stakes) for an event offered by the bookmaker.
...
```{r data-return-summary-table3, echo = FALSE, results = 'asis'}
## Load package again due to cannot find the function
suppressMessages(require('formattable', quietly = TRUE))
...
*table 4.3.1* : `r paste0(dim(m), collapse=' x ')` : *Return of annually investment summary table without cancelled bets.*^[the `rRates` is the mean value of annual return rates which is the return divides by stakes but ommit the cancelled/voided bets to avoind the bias.]
...
$$\rho_{i}^{em} = \frac{S + 1} {BK_{Decimal\ odds} - 1} \cdots equation 4.3.2$$
...
##
## 1. test the staking strategy 1
dat %<>% mutate(KStakes = Stakes / HKPrice, KReturn = ifelse(Result == 'Win', KStakes * EUPrice, ifelse(Result == 'Half Win', KStakes * (HKPrice * 0.5 + 1), ifelse(Result == 'Push', KStakes, ifelse(Result == 'Half Loss', KStakes * 0.5, ifelse(Result == 'Loss', 0, NA))))))
##  A tibble: 5 × 8
#      Result    KReturn     Return  EUPrice   HKPrice  KStakes   Stakes  Rebates
#      <fctr>      <dbl>      <dbl>    <dbl>     <dbl>    <dbl>    <dbl>    <dbl>
# 1 Half Loss   74025.56   68417.25 1.968293 0.9680931 148051.1 136834.5  -146.21
# 2  Half Win  246836.14  215794.35 1.905279 0.9051838 172308.5 149055.2   142.41
# 3      Loss       0.00       0.00 1.971603 0.9714477 748648.7 688224.8 -1466.46
# 4      Push  237270.20  204866.50 1.935221 0.9351197 237270.2 204866.5     0.00
# 5       Win 1698603.65 1534196.76 1.925482 0.9253417 898507.3 799959.3  1565.85

colSums(dat[c('KReturn', 'Return', 'EUPrice', 'HKPrice', 'KStakes', 'Stakes', 'Rebates')])
#    KReturn     Return    EUPrice    HKPrice    KStakes     Stakes    Rebates 
# 2256735.55 2023274.85   94322.23   45797.30 2204785.82 1978940.43      95.59
## 1346719.75/1317252.44 = 1.02237, 2023274.85/1978940.43 = 1.022403

## 2. test the staking strategy 2
dat %<>% mutate(KStakes = (Stakes * EUPrice + 1) / (EUPrice + 1), KReturn = ifelse(Result == 'Win', KStakes * EUPrice, ifelse(Result == 'Half Win', KStakes * (HKPrice * 0.5 + 1), ifelse(Result == 'Push', KStakes, ifelse(Result == 'Half Loss', KStakes * 0.5, ifelse(Result == 'Loss', 0, NA))))))
##  A tibble: 5 × 8
#      Result    KReturn     Return  EUPrice   HKPrice   KStakes   Stakes  Rebates
#      <fctr>      <dbl>      <dbl>    <dbl>     <dbl>     <dbl>    <dbl>    <dbl>
# 1 Half Loss   45713.79   68417.25 1.968293 0.9680931  91427.57 136834.5  -146.21
# 2  Half Win  143060.39  215794.35 1.905279 0.9051838  98662.19 149055.2   142.41
# 3      Loss       0.00       0.00 1.971603 0.9714477 459698.15 688224.8 -1466.46
# 4      Push  135554.77  204866.50 1.935221 0.9351197 135554.77 204866.5     0.00
# 5       Win 1022390.80 1534196.76 1.925482 0.9253417 531909.77 799959.3  1565.85

colSums(dat[c('KReturn', 'Return', 'EUPrice', 'HKPrice', 'KStakes', 'Stakes', 'Rebates')])
#    KReturn     Return    EUPrice    HKPrice    KStakes     Stakes    Rebates 
# 1346719.75 2023274.85   94322.23   45797.30 1317252.44 1978940.43      95.59
## 2256735.55/2204785.82 = 1.023562, 2023274.85/1978940.43 = 1.022403
...
*table 4.3.2* : `r paste0(dim(m), collapse=' x ')` : *Kelly stakes and firm A's stakes of annually investment summary table without cancelled bets.*^[The annum return rates but not all data return rates since we need to use the return of that particular year to reverse. The weight parameter in later section will take last year performance to be weight controller. Due to the firm A does not only placed bets with one agent but couples of agencies. Let say placed one millions HKD on a soccer match with higher price simultaniously compare to placed bets on one agency with price decreasingly, therefore the Kelly stakes will definately defferent with firm A's stakes.]

  Above Kelly stakes table 

```{r}
ddply(K, .(Result), summarise, KReturn = sum(KReturn), Return = sum(Return), EUPrice = mean(EUPrice), HKPrice = mean(HKPrice), KStakes = sum(KStakes), Stakes = sum(Stakes), Rebates = sum(Rebates), Change = mean(Change)) %>% tbl_df
```
...
preData[duplicated(preData[c('Date', 'Home', 'Away')]), c('Date', 'Home', 'Away', 'HG', 'AG', 'InPlay', 'InPlay2', 'Mins', 'Mins2', 'Picked2')] %>% %>% formattable %>% as.htmlwidget
```
