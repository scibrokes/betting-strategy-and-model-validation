...
# 4. Staking Ⓜodel
  ...
  - Section [4.6 Expectation Ⓜaximization and Staking Simulation] - Enhance by weighted function on Staking model and Simulation.
    ...
    + Section [4.6.2 Resampling Scores and Stakes] - Application of Monte Carlo Method to get the optimal return from investment.
...
#'@ save(K1mean1, file = './KellyApps/K1mean1.rda')
load('./KellyApps/K1mean1.rda')
...
## save above data to easier for loading.
#'@ save(K1mean2, file = './KellyApps/K1mean2.rda')
load('./KellyApps/K1mean2.rda')
...
## save above data to easier for loading.
#'@ save(K2mean1, file = './KellyApps/K2mean1.rda')
load('./KellyApps/K2mean1.rda')
...
## save above data to easier for loading.
#'@ save(K2mean2, file = './KellyApps/K2mean2.rda')
load('./KellyApps/K2mean2.rda')
...
KCS1 <- read_rds(path = './data/K1.rds')$Kelly1 #Kelly 1 Candle Stick model 1

## directly load internal function in compareKelly.
suppressAll(source('./function/plotChart.R'))
plotChart(Fund = KCS1, type = 'multiple')
```

*graph 4.3.5.1 : Daily bank roll for Basic Kelly model.*
...
*table 4.5.1.1* : `r paste0(dim(K), collapse = ' x ')` : *Kelly stakes and firm A's stakes of annually investment summary table.*

### 4.5.2 Investors' Fund Refill

  Kelly model will be a good risk averse investment portfolio. However, as we know normally a mutual fund or any investment fund will advise investors credit a certain money into the pool regularly. For this section I keep it as the next study which is in [Application of Kelly Criterion model in Sportsbook Investment - Part II](https://github.com/scibrokes/kelly-criterion). It will includes the :
...
par(mfrow = c(2, 4))
plot(sc0, main = 'raw', pch = '+')
plot(sc1, main = 'option1', pch = '+')
```
...
*table 4.5.2.1A* : `r paste0(dim(scs.m), collapse = ' x ')` : *Comparison of truncated bivariate normal distributions part-1.*
...
*table 4.5.2.1B* : `r paste0(dim(scs.v), collapse = ' x ')` : *Comparison of truncated bivariate normal distributions part-2.*
...
```
  ...
  You are feel free to refer to below articles for further understanding:
