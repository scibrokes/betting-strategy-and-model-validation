...
#'@ list(lm0, lm1, lm2, lm3, lm4, lm5, lm6, lm7, lm8, lm9, lm10, lm11, lm12) %>% llply(., stargazer, type = 'html')
stargazer(lm0, lm1, lm2, lm3, lm4, lm5, lm6, lm7, lm8, lm9, lm10, lm11, lm12, type = 'html')

#'@ list(lm0, lm1, lm2, lm3, lm4, lm5, lm6, lm7, lm8, lm9, lm10, lm11, lm12) %>% llply(., texreg)
```

*table 4.2.1 : Summary of linear models.*

```{r lm-anova, echo = FALSE, results = 'asis'}
## xtable always shows LaTeX output but not table.
...
#'@ list(lm0, lm1, lm2, lm3, lm4, lm5, lm6, lm7, lm8, lm9, lm10, lm11, lm12) %>% llply(., function(x) stargazer(anova(x), type = 'html'))

#'@ stargazer(anova(lm0, lm1, lm2, lm3, lm4, lm5, lm6, lm7, lm8, lm9, lm10, lm11, lm12), type = 'html')
...
#'@ list(lm0, lm1, lm2, lm3, lm4, lm5, lm6, lm7, lm8, lm9, lm10, lm11, lm12) %>% llply(., function(x) texreg(anova(x)))
```
...
shinyApp(
  ...
  ui = fluidPage(
    ...
    sidebarLayout(
      sidebarPanel(
        selectInput('selection', 'Choose an output:',
                    choices = list('summary', 'anova')),
        actionButton('update', 'Display')
        ),
      
      mainPanel(
        tabsetPanel(
          tabPanel('Summary', renderUI('txte')),
          tabPanel('Anova 01~04', renderUI('wordplot')),
          tabPanel('Anova 05', renderUI('table')),
          tabPanel('Anova 06', renderUI('table')),
          tabPanel('Anova 07', renderUI('table')),
          tabPanel('Anova 08', renderUI('table')),
          tabPanel('Anova 09', renderUI('table')),
          tabPanel('Anova 10', renderUI('table')),
          tabPanel('Anova 11', renderUI('table')),
          tabPanel('Anova 12', renderUI('table')),
          tabPanel('Reference', h4('Reference:'),
                   ...
                   p('1. ', HTML("<a href='http://www.r-bloggers.com/getting-started-with-mixed-effect-models-in-r/'>Getting Started with Mixed Effect Models in R</a>")),
                   p('1. ', HTML("<a href='...'>A very basic tutorial for performing linear mixed effects analyses</a>")),
                   p('1. ', HTML("<a href='...'>Linear Models with R</a>")),
                   p('1. ', HTML("<a href='...'>Extending the Linear Model with R : Generalized Linear, Mixed Effects and Nonparametric Regression Models</a>"))
                   )
        )
...
```{r chiny-calc02, echo = FALSE, results = 'asis'}
