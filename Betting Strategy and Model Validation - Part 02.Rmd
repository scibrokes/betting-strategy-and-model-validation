      ...
      ui = fluidPage(
        ...
        sidebarLayout(
          sidebarPanel(
            ...
            checkboxInput('type', label = h4('Summary / Anova?'), choices = list('Summary', 'Anova')),
            checkboxGroupInput('models', label = h4('Which models?'), choices = names(lms)),
      ...
      server = function(input, output) {
        observeEvent(input$analysis, {
          ...
          output$modelSummary = renderPrint({
            list(summary(model), anova(model))
            })
          })
        
        model0 <- renderPrint(list(summary(lm0), anova(lm0, lm1, lm2, lm3, lm4)))
        model1 <- renderPrint(list(summary(lm1), anova(lm0, lm1, lm2, lm3, lm4)))
        model2 <- renderPrint(list(summary(lm2), anova(lm0, lm1, lm2, lm3, lm4)))
        model3 <- renderPrint(list(summary(lm3), anova(lm0, lm1, lm2, lm3, lm4)))
        model4 <- renderPrint(list(summary(lm4), anova(lm0, lm1, lm2, lm3, lm4)))
        model5 <- renderPrint(list(summary(lm5), anova(lm5)))
        model6 <- renderPrint(list(summary(lm6), anova(lm6)))
        model7 <- renderPrint(list(summary(lm7), anova(lm7)))
        model8 <- renderPrint(list(summary(lm8), anova(lm8)))
        model9 <- renderPrint(list(summary(lm9), anova(lm9)))
        model10 <- renderPrint(list(summary(lm10), anova(lm10)))
        model11 <- renderPrint(list(summary(lm11), anova(lm11)))
        model12 <- renderPrint(list(summary(lm12), anova(lm12)))
        model13 <- renderPrint(list(summary(lm13), anova(lm13)))
        model14 <- renderPrint(list(summary(lm14), anova(lm14)))
        }
      )
    )
```
...
## 4.4 Poisson Ⓜodel
  ...
  *Niko Marttinen (2001)*^[Kindly refer to 1th paper in [Reference for industry knowdelege and academic research portion for the paper.]] has enhanced the *Dixon and Coles (1996)*^[Kindly refer to 25th paper in [Reference for industry knowdelege and academic research portion for the paper.]] which are :
...
![*figure 4.4.1 : Comparison of various Poison models.*](figure/NikoMarttinen2001-01.jpg)
  ...
  From *figure 4.4.1* above, the author 

  - Weighted model : 
  
  Here we introduce the *Dixon and Coles (1996)*^[Kindly refer to 25th paper in [Reference for industry knowdelege and academic research portion for the paper.]] Poisson model and its codes. You are freely learning from below links if interest.
