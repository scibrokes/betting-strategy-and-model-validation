...
dat$rRates <- rep(m$rRates, ddply(dat, .(Sess), summarise, n = length(Sess))$n)
...
lRiskProf <- dat[c('League', 'Stakes', 'HCap', 'Result', 'PL')] %>% tbl_df %>% .[sort(.$League), ] %>% unique %>% na.omit %>% mutate(Stakes = currency(Stakes), PL = ifelse(PL < 0, paste0('-$', substring(PL, 2)), paste0('$', PL)))
...
dat %<>% mutate(
  KProb = ((Stakes / 10000) * HKPrice + 1) / EUPrice, 
...
kable(summary(Kprof), caption = 'Table 4.3.3.2 : Summary Table of Various Kelly Models')
```

*table 4.3.3.2* : `r paste0(dim(Kprof), collapse = ' x ')` : *League stakes profiling of firm A from 2011~2015.*
...
*table 4.3.3.3* : `r paste0(dim(sumd), collapse = ' x ')` : *P&L of multiple Kelly models from 2011~2015.*
...
*table 4.3.3.1* : `r paste0(dim(K), collapse = ' x ')` : *Kelly stakes and firm A's stakes of annually investment summary table without cancelled bets.*^[The annum return rates but not all data return rates since we need to use the return of that particular year to reverse. The weight parameter in later section will take last year performance to be weight controller. Due to the firm A does not only placed bets with one agent but couples of agencies. Let say placed one millions HKD on a soccer match with higher price simultaniously compare to placed bets on one agency with price decreasingly, therefore the Kelly stakes will definately defferent with firm A's stakes.]
...
*table 4.3.1.2* : `r paste0(dim(BR), collapse = ' x ')` : *Sample data of daily bank roll. (HKD$1 = HKD$10,000)*
