...
>  Due to the coefficient of the regression models ocuppied a long space in this article. Here I write in a shiny app which perform the summary and anova of the models. **Kindly click on [*Report with ShinyApps*](https://beta.rstudioconnect.com/connect/1766) to use the ShinyApp**. It will take few minutes time to open the website since the dataset contain 5 years soccer data from firm A staking via agent A made it a heavily loading.
...
```{r shiny-summary, eval = FALSE, echo = FALSE, results = 'asis'}
## completed verson kindly refer to https://beta.rstudioconnect.com/content/1807/
...
```{r shiny-calc01, eval = FALSE, echo = FALSE, results = 'asis'}
## completed verson kindly refer to https://beta.rstudioconnect.com/content/1807/
...
$$P = \rho_{i}... \rho_{n} \cdots equation 4.2.1$$

Where $0 < \rho_{i} < 1$ and $i = 1,2,3,...n$ as well as $\sum_{i=1}_{n} \rho_{i} = 1$

  - the beliefs of a punter with positive inside information about staked team are represented by the vector
  
$$(1 - \alpha)P + \alphae_{i} \cdots equation 4.2.2$$

where $e_{i} = (0, ..., 0, 1, 0, ..., 0)$ is the degenerate distribution
assigning all probability mass to the $i$th team and $0 < \alpha <= 1$ and
