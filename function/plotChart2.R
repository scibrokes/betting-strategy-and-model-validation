plotChart2 <- function(Fund, type = 'multiple', event = NULL, event.dates = NULL, 
  ...
  if(type == 'single') {
    ...
    if(chart.type == 'Hi') {
      FUND.SMA.10  <- SMA(Hi(Fund), n = 10)
      FUND.SMA.200 <- SMA(Hi(Fund), n = 200)
      FUND.RSI.14  <- RSI(Hi(Fund), n = 14)
      FUND.RSI.SellLevel <- xts(rep(70, NROW(Fund)), index(Fund))
      FUND.RSI.BuyLevel  <- xts(rep(30, NROW(Fund)), index(Fund))
      
      initial <- Hi(Fund)[1, ] %>% unique %>% currency
      
      fname <- names(Hi(Fund)) %>% str_replace_all('.High', '')
    }
    
    if(chart.type == 'Lo') {
      FUND.SMA.10  <- SMA(Lo(Fund), n = 10)
      FUND.SMA.200 <- SMA(Lo(Fund), n = 200)
      FUND.RSI.14  <- RSI(Lo(Fund), n = 14)
      FUND.RSI.SellLevel <- xts(rep(70, NROW(Fund)), index(Fund))
      FUND.RSI.BuyLevel  <- xts(rep(30, NROW(Fund)), index(Fund))
      
      initial <- Lo(Fund)[1, ] %>% unique %>% currency
      
      fname <- names(Lo(Fund)) %>% str_replace_all('.Low', '')
    }
    ...
    if(chart.type == 'Cl') {
      ...
      initial <- Cl(Fund)[1, ] %>% unique %>% currency
      
      fname <- names(Cl(Fund)) %>% str_replace_all('.Close', '')
    }
    ...
    if(chart.type == 'Hi') {
      ...
      fname <- grep('.High', names(Hi(Fund)), value = TRUE) %>% 
        str_split('\\.') %>% llply(., function(x) 
          paste0(str_replace_all(x, 'High', '')[1:2], collapse = '.')) %>% unlist
      
      ## add dates for event...
      ##   label the high volatility daily event.
      if(is.null(event.dates)) {
        event.dates <- as.Date(period.apply(
          diff(Hi(Fund)), INDEX = endpoints(Fund), FUN = max) %>% data.frame %>% 
            rownames, format = '%Y-%m-%d')
      } else {
        event.dates <- as.Date(event.dates)
      }
    ...
    if(chart.type == 'Lo') {
      ...
      fname <- grep('.Low', names(Lo(Fund)), value = TRUE) %>% 
        str_split('\\.') %>% llply(., function(x) 
          paste0(str_replace_all(x, 'Low', '')[1:2], collapse = '.')) %>% unlist
      
      ## add dates for event...
      ##   label the high volatility daily event.
      if(is.null(event.dates)) {
        event.dates <- as.Date(period.apply(
          diff(Lo(Fund)), INDEX = endpoints(Fund), FUN = max) %>% data.frame %>% 
            rownames, format = '%Y-%m-%d')
      } else {
        event.dates <- as.Date(event.dates)
      }
