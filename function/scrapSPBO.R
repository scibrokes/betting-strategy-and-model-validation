scrapSPBO <- function(lnk = lnk, dateID = dateID, path = path, parallel = FALSE){
  ...
  llply(n, function(i){
    ...
    if((dfm[, 04] == 0) & (dfm[, 05] == 0)){
      dfm[(dfm[, 04] == 0) & (dfm[, 05] == 0), 
          c((ncol(dfm) - 8), (ncol(dfm) - 5), (ncol(dfm) - 3):ncol(dfm))] <- 
    ...
    if((dfm[, 04] != 0) & (nchar(dfm[, 04]) == 1)){
      dfm[(dfm[, 04] != 0) & (nchar(dfm[, 04]) == 1), 5:ncol(dfm)] <- 
        dfm[(dfm[, 04] != 0) & (nchar(dfm[, 04]) == 1), 4:14]
    }
    if((nchar(dfm[, 04]) > 1) & (nchar(dfm[, 07]) == 1)){
      dfm[(nchar(dfm[, 04]) > 1) & (nchar(dfm[, 07]) == 1), 07:ncol(dfm)] <- 
        c(0, dfm[(nchar(dfm[, 04]) > 1) & (nchar(dfm[, 07]) == 1), 7:14])
    }
    if((nchar(dfm[, 04]) > 1) & (nchar(dfm[, 10]) == 1)){
      dfm[(nchar(dfm[, 04]) > 1) & (nchar(dfm[, 10]) == 1), 10:ncol(dfm)] <- 
        c(0, dfm[(nchar(dfm[, 04]) > 1) & (nchar(dfm[, 10]) == 1), 10:14])
    }
    ...
    dfnames <- c('matchID', 'LeagueColor', 'League', 'Date', 'Finished', 'Home', 'Away', 
                 'FTHG', 'FTAG', 'HTHG', 'HTAG', 'H.Card', 'A.Card', 
    ...
    dfm %<>% mutate(Home = factor(as.character(gsub('^.*;', '', as.character(Home)))),
    ...
    if(dir.exists('./data/livescore') == FALSE){
      dir.create(file.path('./data/', path))
    }
    ...
    write.csv(dfm, file = paste0('./data/', path, '/', dateID[i], '.csv'))
    cat(i, paste0(' Wrote ', dateID[i], '.csv'), '\n')}, .parallel = parallel)
  
  ## zip the dataset s. 
  zip('./data/livescore.zip', './data/livescore')
