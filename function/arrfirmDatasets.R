  ...
  if((is.error(dfm))|(is.null(dfm))){
    ...
    dfm %<>% mutate(fMYPriceB=ifelse(HKPrice > 1, round(-1/(HKPrice), 3), round(HKPrice, 3)),
                    fMYPriceL=ifelse(fMYPriceB < 0 & AHOU == 'AH', abs(fMYPriceB + lProfile[1]),
                              ifelse(fMYPriceB <= 1 & fMYPriceB > lProfile[1] & AHOU == 'AH', 2-fMYPriceB - lProfile[1],
                              ifelse(fMYPriceB <= 1 & fMYPriceB > lProfile[2] & AHOU == 'OU', 2-fMYPriceB - lProfile[2],
    ...
    dfm %<>% mutate(pHKRange=cut(HKPrice, seq(floor(min(mPrice$HKPrice)), ceiling(max(mPrice$HKPrice)), 0.1)), 
                    ...
                    pMYRange=cut(fMYPriceB, seq(floor(min(mPrice$fMYPriceB)), ceiling(max(mPrice$fMYPriceB)), 0.1)))
    ...
    ##
    dfm %<>% mutate(InPlay=factor(ifelse(str_detect(Mins, 'ET'), 'ET', ifelse(str_detect(Mins, 'No'), 'No', 'FT'))),
    ...
    dfm %<>% mutate(FHFTET=factor(ifelse(str_detect(Home, '1st Half'), 'FH',
                   ...
                   Picked2=factor(ifelse(as.character(Selection) == as.character(Home), 'home',
                                  ifelse(as.character(Selection) == as.character(Away), 'away', as.character(Picked)))),
