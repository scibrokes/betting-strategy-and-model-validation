KellyPL <- function(K, type = 'vKelly', adjusted = 1) {
  ...
  ## --------------------- Data validation --------------------------------------
  if(!is.data.frame(K)) {
    stop('KellyPL() is an internal function applicable to vKelly() and vKelly2().')
  }
  ...
  if(type == 'vKelly') {
    ...
    ## ---------------- K Quantmod Candle Stick Format ----------------------
  ...
  ## ---------------- vKelly2 ----------------------
    ...
    ## ---------------- Return Function ----------------------
    options(warn = 0)
    return(list(data = K, summary = sumdf, BR = BR, initial = initial, adjusted = adjusted))
    
  } else {
    stop('Kindly input type = "vKelly" or type = "vKelly2".')
  }
}