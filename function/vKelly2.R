vKelly2 <- function(mbase, weight.stakes = 1, weight = 1) {
  ...
  K1 %<>% mutate(
    ...
    KStakesHKPrice = ((weight.stakes * rEMProbB) * EUPrice - 1) / HKPrice, 
    
    KStakesnetProbB = ((weight.stakes * rEMProbB) * (1 / netProbB) - 1) / (1 / netProbB - 1),
    
    KStakesFixed = exp((log(weight.stakes * rEMProbB) * EUPrice - 1) / HKPrice), 
    
    KStakesFixednetProbB = exp(((log(weight.stakes * rEMProbB)) * (1 / netProbB) - 1) / (1 / netProbB - 1)),
    
    KStakesEMProb = (weight.stakes * rEMProbB * EUPrice - 1) / HKPrice, 
    
    KStakesEMProbnetProbB = (weight.stakes * rEMProbB *  (1 / netProbB) - 1) / (1 / netProbB - 1),
    
    KStakesHalf = ((weight.stakes * 0.5 * rEMProbB) * EUPrice - 1) / HKPrice, 
    
    KStakesHalfnetProbB = (weight.stakes * (0.5 * rEMProbB) * (1 / netProbB) - 1) / (1 / netProbB - 1), 
    
    KStakesQuarter = (weight.stakes * (0.25 * rEMProbB) * EUPrice - 1) / HKPrice, 
    
    KStakesQuarternetProbB = ((0.25 * rEMProbB) * (1 / netProbB) - 1) / (1 / netProbB - 1), 
    
    KStakesAdj = exp((log(weight.stakes * rEMProbB) * EUPrice - 1) / HKPrice), 
    
    KStakesAdjnetProbB = exp((log(weight.stakes * rEMProbB) * (1 / netProbB) - 1) / (1 / netProbB - 1)), 
    
    KStakesHalfAdj = exp((log(weight.stakes * 0.5 * rEMProbB) * EUPrice - 1) / HKPrice), 
    
    KStakesHalfAdjnetProbB = exp((log(weight.stakes * 0.5 * rEMProbB) * (1 / netProbB) - 1) / (1 / netProbB - 1)), 
    
    KStakesEMQuarterAdj = exp((log(weight.stakes * 0.25 * rEMProbB) * EUPrice - 1) / HKPrice), 
    
    KStakesEMQuarterAdjnetProbB = exp((log(weight.stakes * 0.25 * rEMProbB) * (1 / netProbB) - 1) / (1 / netProbB - 1))
  ...
  if(all(wt$weight.stakes) != 1 & all(wt$weight) != 1) {
    ...
    K2 %<>% mutate(
      ...
      KStakesFixed = weight.stakes * exp((log(rEMProbB) * EUPrice - 1) / HKPrice), 
      
      KStakesFixednetProbB = weight.stakes * exp(((log(rEMProbB)) * (1 / netProbB) - 1) / (1 / netProbB - 1)),
      ...
      KStakesQuarternetProbB = weight.stakes * ((0.25 * rEMProbB) * (1 / netProbB) - 1) / (1 / netProbB - 1), 
      
      KStakesAdj = weight.stakes * exp((log(rEMProbB) * EUPrice - 1) / HKPrice), 
      
      KStakesAdjnetProbB = weight.stakes * exp((log(rEMProbB) * (1 / netProbB) - 1) / (1 / netProbB - 1)), 
      
      KStakesHalfAdj = weight.stakes * exp((log(0.5 * rEMProbB) * EUPrice - 1) / HKPrice), 
      
      KStakesHalfAdjnetProbB = weight.stakes * exp((log(0.5 * rEMProbB) * (1 / netProbB) - 1) / (1 / netProbB - 1)), 
      
      KStakesEMQuarterAdj = weight.stakes * exp((log(0.25 * rEMProbB) * EUPrice - 1) / HKPrice), 
      
      KStakesEMQuarterAdjnetProbB = weight.stakes * exp((log(0.25 * rEMProbB) * (1 / netProbB) - 1) / (1 / netProbB - 1))
