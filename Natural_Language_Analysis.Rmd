...
df1 <- data_frame(team=teamID[tolower(teamID) %in% tolower(spboTeamID)], spbo=spboTeamID[tolower(spboTeamID) %in% tolower(teamID)]) %>% mutate(team=as.character(team),spbo=as.character(spbo),pass=ifelse(team==spbo, 'Duplicated', 'Capital Letters')) %>% arrange(pass)
row.names(df1) <- NULL
...
tmIDdata <- read.csv(paste0(getwd(), '/datasets/teamID.csv'), header=TRUE, sep=', ') %>% data_frame %>% mutate_each(funs(as.character))
...
comp1 <- tmIDdata %>% filter(., .$teamID %in% strDF$result$teamID & !duplicated(.$teamID)) %>% merge(., strDF$result) %>% data_frame %>% .[c('teamID', 'spbo', method)] %>% mutate_each(funs(as.character))
...
data_frame(match=names(res1), rate=res1/res1[1], n=res1) %>% kable(caption='Table 5.2.2 : Summary of Matching Result 1')
```
  
  Same with above, we simply filter the `PartialMatch` function.
  
```{r compare-datasetC, echo=FALSE, results='asis'}
comp2 <- tmIDdata %>% subset(., .$teamID %in% df4$teamID & !duplicated(.$teamID)) %>% merge(.,df4) %>% data_frame %>% .[c('teamID', 'spbo', 'spboID')] %>% mutate_each(funs(as.character))
...
data_frame(match=names(res2), rate=res2/res2[1],n=res2) %>% kable(caption='Table 5.2.4 : Summary of Matching Result 2')
```
